...
void TimedRobot::start_competition(){
  ...
  while(1){
    was_delayed = xTaskDelayUntil( &last_wake, pdMS_TO_TICKS(this->delta_time_ms) );
    this->run_tick();
    if(!was_delayed){
...
void TimedRobot::run_tick(){
  ...
  if(this->first || mode.enabled != this->current_mode.enabled || mode.mode != this->current_mode.mode){
