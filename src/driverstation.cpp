...
void Driverstation::observe_disabled()
{
    observe_mode(255);
}

void Driverstation::observe_teleop()
{
    observe_mode(0);
}

void Driverstation::observe_test()
{
    observe_mode(1);
}

void Driverstation::observe_auton()
{
    observe_mode(2);
}
...
void Driverstation::print(std::string msg)
{
    print(msg.c_str(), msg.size());
}

void Driverstation::print(const char *msg)
{
...
void Driverstation::update_control_mode(bool enabled, uint8_t mode, ControlCode initial)
{
    auto updated = initial;
    do
    {
...
bool Driverstation::read_controller_tag(uint8_t data[], uint len, int controller)
{
    xSemaphoreTake(this->udp_controller_mutex, portMAX_DELAY);
    if (len == 0)
    {
