...
DSToRobotUDP DSToRobotUDP::from_big_endian_buffer(uint8_t *buf, size_t len,
                                                  size_t capacity) {
  if (capacity < 6) {
    return {};
  }
...
size_t DSToRobotUDP::tags_len() { return this->size - 6; }
...
RobotToDSUDP RobotToDSUDP::from_big_endian_buffer(uint8_t *buf, size_t len,
                                                  size_t capacity) {
  if (capacity < 8) {
    return {};
  }
...
RobotToDSUDP RobotToDSUDP::from_little_endian_buffer(uint8_t *buf,
                                                     size_t capacity) {
  if (capacity < 8) {
    return {};
  }
...
size_t RobotToDSUDP::tags_len() { return this->size - 8; }
